...
out = np.zeros((image_bordered.shape[0],image_bordered.shape[0]), dtype=np.float32)
o = np.zeros((img.shape[0],img.shape[0]), dtype=np.float32)
print(pi)
...
for i in range (x-1,img.shape[0]-x):
    for j in range (y-1,img.shape[1]-y):
        s=0
